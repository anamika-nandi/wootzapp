...
export function PartnerShip() {
	return (
		<div className='min-h-screen relative md:pt-24 pb-16 overflow-hidden'>
			<img src='img/partnership-shadow.png' alt='Decorative shadow' className='absolute -top-10 -right-16 md:-right-44 w-40' />
				...
				<div className='flex flex-col mt-16'>
						...
						{data.map((e, index) => {
							return (
								<div key={index} className='flex bg-brand-white-100 p-5 gap-3 lg:gap-4 rounded-md basis-full z-10'>
									<img src={e.img} alt={`Icon for ${e.head}`} className='object-cover ~size-14/20' />
					...
					<div className='flex flex-col-reverse md:flex-row items-center mt-5 md:mt-14 justify-center gap-5 md:gap-10 lg:gap-16'>
						<div className='max-w-2xl flex-1'>
							<img src='img/partnership.png' alt='Partnership illustration' className='w-full' />
						</div>

						<div className='bg-[#EEEFE9] rounded-md flex flex-1 w-full h-full p-5 gap-2 md:gap-5'>
							<div>
								<img src='img/code.png' alt='Code icon' className='w-16 md:w-24' />
							</div>
...
async function deployWorkflow(extension: WootzExtension) {
  ...
  return deployment.monitor();
							...
							{/* <img src='img/preview.png' alt='Code preview' className='w-full hidden md:block' />

							<img src='img/preview-mobile.png' alt='Mobile code preview' className='w-full md:hidden' /> */}
						</div>
					</div>
				</div>
			</div>

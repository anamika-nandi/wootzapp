		...
		<div className={`sticky top-0 z-[1000] transition-all duration-300 ${isScrolled ? "bg-brand-home_bg shadow-md" : "bg-transparent"}`}>
			<div className='flex justify-between container items-center py-5'>
				...
				<div className='hidden md:flex gap-10'>
					{headerData.map(data => (
						<a className='font-matter font-normal text-brand-black-200 hover:text-inherit hover:no-underline text-base xl:text-xl' key={data} href='#'>
							{data}
						</a>
					))}
				</div>
				...
				<button className='md:hidden p-2 focus:outline-none' onClick={toggleMobileMenu} aria-label='Toggle mobile menu'>
					{isMobileMenuOpen ? <AiOutlineClose size={24} className='text-brand-black-200' /> : <MdMenu size={24} className='text-brand-black-200' />}
				</button>
			</div>
							...
							{headerData.map((data, index) => (
								...
								>
									...
									<svg width='8' height='12' viewBox='0 0 8 12' fill='none' xmlns='http://www.w3.org/2000/svg'>
										<path d='M1.5 11L6.5 6L1.5 1' stroke='#333' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round' />
									</svg>
								</a>
							))}
						</div>
